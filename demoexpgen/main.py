...
def callLD():
    ...
    for i in range(NUMBER_OF_ITERATIONS):
        ...
        if execute_call_if_converted(
            funnel_metric_1, funnel_metric_1_percent_converted, context
        ):
            if index == 0:
                print("Serving Control")
                # Track latency
                latency = random.randint(
                    control_ai_response_latency_range[0],
                    control_ai_response_latency_range[1],
                )
                ldclient.get().track(
                ...
                # Track cost
                cost = random.uniform(
                    control_ai_cost_range[0], control_ai_cost_range[1]
                )
                ldclient.get().track("ai-analysis-cost", context, metric_value=cost)
                # Track CSAT
                calc_csat(
                    control_ai_csat_positive_percent_converted,
                    control_ai_csat_negative_percent_converted,
                    context,
                )
                # Track funnel metric 2 if they convert
                execute_call_if_converted(
                    funnel_metric_2, control_funnel_metric_2_percent_converted, context
                )
                ...
                # Track latency
                latency = random.randint(
                    t1_ai_response_latency_range[0], t1_ai_response_latency_range[1]
                )
                ldclient.get().track(
                    "ai-response-latency", context, metric_value=latency
                )
                ...
                # Track CSAT
                calc_csat(
                    t1_ai_csat_positive_percent_converted,
                    t1_ai_csat_negative_percent_converted,
                    context,
                )
                # Track funnel metric 2 if they convert
                execute_call_if_converted(
                    funnel_metric_2, t1_funnel_metric_2_percent_converted, context
                )
                ...
                # Track latency
                latency = random.randint(
                    t2_ai_response_latency_range[0], t2_ai_response_latency_range[1]
                )
                ldclient.get().track(
                    "ai-response-latency", context, metric_value=latency
                )
                ...
                # Track CSAT
                calc_csat(
                    t2_ai_csat_positive_percent_converted,
                    t2_ai_csat_negative_percent_converted,
                    context,
                )
                # Track funnel metric 2 if they convert
                execute_call_if_converted(
                    funnel_metric_2, t2_funnel_metric_2_percent_converted, context
                )
            else:
                pass
